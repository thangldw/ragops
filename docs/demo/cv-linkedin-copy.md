# CV and LinkedIn copy

Use after owner review; these statements describe public repository evidence,
not customer production adoption.

## CV project entry

**RAGOps — Open-source evaluation and release infrastructure for RAG/agents**

- Designed and shipped a dependency-free Python evaluation core with versioned
  scenarios/traces, regression gates, red-team policies, CLI/API, CI, Docker,
  experiment review/trends, and a workspace control-plane alpha.
- Built a 30-case Japanese enterprise benchmark and ACL-first graph-assisted
  troubleshooting reference agent; RAGOps blocked a lexical-only candidate with
  25-point citation coverage/precision regressions.
- Published architecture decisions, threat model, rollout plan, executive
  go/no-go recommendation, reproducible reports, and 99 automated tests.

## LinkedIn project description

RAGOps demonstrates my Forward Deployed Engineering approach: start with an
ambiguous Japanese enterprise workflow, define measurable release criteria,
build a production-shaped reference system, test regressions and safety, and
make an honest rollout decision. The repository and case study are public.

Repository: https://github.com/thangldw/ragops
Case study: https://thangldw.github.io/projects/ragops/
