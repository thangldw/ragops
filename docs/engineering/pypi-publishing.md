...
- Current verified release: `2.4.0`
- GitHub environment: `pypi`
- Publisher: `thangldw/ragops`, workflow `publish-pypi.yml`
- Verification: 2.4.0 wheel and sdist match the GitHub Release byte-for-byte;
  clean install and credential-free `ragops demo` pass
- v2.4 publication path: project-scoped-token local fallback while GitHub
  Actions quota was unavailable; `trusted_publishing` is therefore false for
  this release even though the preferred workflow remains OIDC
...
2. Enter an existing tag such as `v2.4.0`.
